...
export default function OfferCreate() {
  ...
  useEffect(() => {
    const raw = localStorage.getItem("mood-edit-offer");
    if (raw) {
      try {
        const row = JSON.parse(raw);
        const saved = Array.isArray(row.components) ? row.components[0] : null;
        if (saved) {
          // العرض القديم يبقى قابلًا للتعديل على مستوى السعر والبيانات، والمكونات تُعاد بناؤها عند الحاجة.
          setEditingOfferId(Number(row.id));
          setTitle(String(row.title || ""));
          setOfferPrice(String(row.offerPrice || ""));
          setStartsAt(String(row.startsAt || today()));
          setEndsAt(String(row.endsAt || ""));
        }
      } catch {}
      localStorage.removeItem("mood-edit-offer");
    }
    void load();
  }, [effectiveBranchId]);

  async function load() {
    const [loadedMaterials, loadedSizes, loadedBoxes] = await Promise.all([
  ...
  async function saveOffer() {
    ...
    const { error } = editingOfferId
      ? await supabase.from("offers").update(payload).eq("id", editingOfferId).eq("branch_id", effectiveBranchId)
      : await supabase.from("offers").insert(payload);
    setBusy(false);
    if (error) return alert(error.message);
    setTitle(""); setOfferPrice(""); setEditingOfferId(null); chooseType("bouquet"); await load();
    alert(editingOfferId ? "تم تعديل العرض ✅" : "تم حفظ العرض بنفس تفاصيل الطلب ✅");
  }



  return <div dir="rtl" className="space-y-5 p-3 sm:p-6 lg:p-8">
    <header><h1 className="text-2xl font-black sm:text-3xl">إضافة عرض</h1><p className="mt-1 text-sm text-gray-500">ابنِ العرض بنفس مكونات طلب جديد: باقة أو بوكس أو منتج من المخزون.</p></header>
    <section className="rounded-3xl bg-white p-4 shadow-sm sm:p-6">
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-emerald-50 p-4">
        ...
        <button disabled={busy} onClick={()=>void saveOffer()} className="rounded-xl bg-emerald-700 px-7 py-3 font-black text-white disabled:opacity-50">{busy?"جاري الحفظ...":editingOfferId?"حفظ تعديلات العرض":"حفظ العرض"}</button>
      </div>
    </section>
