...
const items: MenuItem[] = [
  ...
  { key: "drafts", label: "📝 مسودات الطلبات" },
  ...
  { key: "treasury", label: "💵 الرصيد الحالي", roles: ["owner", "admin", "manager", "accountant"] },
  { key: "purchase-invoices", label: "🧾 فواتير المشتريات" },
  { key: "waste", label: "🗑️ التوالف والهالك" },
  { key: "expenses", label: "💸 المصروفات" },
  { key: "activity-log", label: "🕘 سجل العمليات", roles: ["owner", "admin"] },
  { key: "whatsapp-campaigns", label: "📣 أرقام واتساب والعروض", roles: ["owner", "admin", "manager"] },
  { key: "whatsapp-logs", label: "📱 سجل واتساب", roles: ["owner", "admin", "manager"] },
  { key: "suppliers", label: "🚚 الموردون" },
  { key: "supplier-reports", label: "📈 تقارير الموردين", roles: ["owner", "admin", "manager", "accountant"] },
  { key: "customers", label: "👥 العملاء" },
  { key: "employees", label: "👨‍💼 الموظفون" },
  { key: "attendance", label: "📍 الحضور والانصراف" },
  { key: "branches", label: "🏢 الفروع", roles: ["owner", "admin", "manager"] },
  { key: "users", label: "🛡️ المستخدمون والصلاحيات", roles: ["owner", "admin"] },
  { key: "reports", label: "📊 التقارير", roles: ["owner", "admin", "manager", "accountant"] },
...
export default function Sidebar({
  ...
  const { selectedBranch, effectiveBranchId, canViewAllBranches } = useBranch();

  const displayName = effectiveBranchId
    ? selectedBranch?.name || "الفرع"
    : canViewAllBranches
      ? "جميع الفروع"
      : selectedBranch?.name || "MOOD";
  ...
  return (
    <>
      ...
      <aside
        ...
        style={{
          background:
            "linear-gradient(to bottom, color-mix(in srgb, var(--branch-primary) 100%, black), color-mix(in srgb, var(--branch-primary) 86%, black))",
        }}
      >
                    ...
                    <button
                      ...
                      style={{
                        backgroundColor: active
                          ? "color-mix(in srgb, var(--branch-primary) 62%, white)"
                          : undefined,
                        boxShadow: active
